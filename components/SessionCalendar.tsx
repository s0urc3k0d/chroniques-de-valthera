  ...
  const SessionCard: React.FC<{ session: typeof sessions[0]; compact?: boolean }> = ({ session, compact = false }) => {
    ...
    return (
      <div
        ...
        className={`
          ${status.bg} border border-slate-700 rounded-lg p-3 cursor-pointer
      ...
      >
        {/* Status badge */}
        {session.status === 'live' && (
          <div className="flex items-center gap-1 text-red-400 text-xs font-bold mb-2 animate-pulse">
        ...
        {/* Titre et campagne */}
        <h4 className={`font-bold text-white ${compact ? 'text-sm' : 'text-lg'} group-hover:text-valthera-400 transition-colors`}>
          {session.title}
        </h4>
        ...
        {compact && (
          <p className="text-xs text-slate-500 mt-1">{formatTime(session.scheduledDate)}</p>
        )}
      </div>
    );
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex bg-slate-800 rounded-lg p-1">
            <button
              ...
              className={`px-3 py-1.5 rounded text-sm transition-colors ${
                view === 'calendar' ? 'bg-valthera-600 text-white' : 'text-slate-400 hover:text-white'
              }`}
            >
              Calendrier
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded text-sm transition-colors ${
                view === 'list' ? 'bg-valthera-600 text-white' : 'text-slate-400 hover:text-white'
              }`}
            >
              Liste
            </button>
          ...
          {editable && onAddSession && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-valthera-600 hover:bg-valthera-500 text-white rounded-lg transition-colors"
            >
              <Plus size={18} />
        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            {upcomingSessions.length === 0 ? (
              <div className="text-center py-8 text-slate-500 glass-panel rounded-xl">
                ...
                {pastSessions.slice(0, 5).map(session => (
                  <div key={session.id} className="relative">
                    <div className={`absolute left-0 top-0 bottom-0 w-1 rounded-full ${
                      session.status === 'cancelled' ? 'bg-slate-600' : 'bg-green-500'
                    }`}></div>
                    <div className="ml-4">
          ...
          >
            ...
            <div className="p-6 space-y-6">
              ...
              {selectedSession.description && (
                <div>
                  <h4 className="text-sm font-bold text-slate-400 uppercase mb-2">📝 Pitch de la session</h4>
                  <p className="text-slate-300">{selectedSession.description}</p>
                </div>
              )}
              ...
              {(selectedSession.twitchLink || selectedSession.youtubeLink) && (
                <div>
                  <h4 className="text-sm font-bold text-slate-400 uppercase mb-2">📺 Diffusion</h4>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {selectedSession.players.length === 0 && (
                    <p className="text-slate-500 text-sm">Aucun joueur inscrit</p>
                  )}
                </div>
              </div>
              ...
              {selectedSession.publicNotes && (
                <div>
                  <h4 className="text-sm font-bold text-slate-400 uppercase mb-2">📌 Notes</h4>
              ...
              {editable && (
                <div className="flex gap-3 pt-4 border-t border-slate-700">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-red-600/20 hover:bg-red-600/40 text-red-400 border border-red-500/50 rounded-lg transition-colors"
                  >
                    <Trash2 size={16} />
                    Supprimer
                  </button>
